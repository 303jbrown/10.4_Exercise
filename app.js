...
const doubleAndReturnArgs = (arr, ...restVal) => ([...arr, ...restVal]);

/** remove a random element in the items array
and return a new array without that item. */

const removeRandom= (items) => {
    let removeItem = Math.floor(Math.random()* items.length)
    let newArr = [...items.slice(0,removeItem),...items.slice(removeItem+1) ]
    return newArr;
